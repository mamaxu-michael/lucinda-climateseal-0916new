...
export default function SolutionResources() {
  ...
  const whitepapers: Whitepaper[] = (articlesData as { whitepapers?: Whitepaper[] }).whitepapers || [];
  ...
  const getWhitepaperTitle = (whitepaper: Whitepaper) => {
    return language === 'zh' ? whitepaper.titleZh : whitepaper.title;
  };

  const getWhitepaperIntro = (whitepaper: Whitepaper) => {
    return language === 'zh' ? whitepaper.introZh : whitepaper.intro;
  };

  const getWhitepaperBenefits = (whitepaper: Whitepaper) => {
    return language === 'zh' ? whitepaper.whatYouGetZh : whitepaper.whatYouGet;
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {whitepapers.map((whitepaper) => {
                ...
                return (
                  <div
                    key={whitepaper.id}
                    className="group bg-white/10 border border-white/10 hover:border-white/30 transition-all duration-300 rounded-2xl p-4 md:p-5 flex flex-col gap-4 lg:flex-row lg:items-start"
                  >
                    <div className="relative w-full lg:w-40 h-40 lg:h-40 rounded-xl overflow-hidden shadow-lg flex-shrink-0">
                      <Image
                      ...
                      />
                      <div className="absolute top-2 left-2 bg-[#9ef894] text-black text-xs font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full">
                    ...
                    <div className="flex-1 space-y-3 min-w-0">
                      <div className="space-y-1">
                        <h3 className="text-lg md:text-xl font-semibold text-white leading-tight font-lora line-clamp-2">
                        ...
                        <ul className="space-y-1 text-xs text-white/80">
                          {displayBenefits.map((benefit, index) => (
                            <li key={index} className="flex items-start gap-2">
                              <span className="mt-0.5 text-[#9ef894] flex-shrink-0">•</span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {whitepaper.topics.slice(0, 2).map((topic) => (
                          <span key={topic} className="bg-white/10 border border-white/10 text-white/70 px-2 py-0.5 rounded-full text-xs uppercase tracking-wide">
                            {topic}
                          </span>
                        ))}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <Link
                          ...
                          className="inline-flex items-center gap-1.5 bg-[#9ef894] text-black px-4 py-2 rounded-full font-semibold text-xs md:text-sm hover:bg-[#8ee884] transition-colors"
                        >
                          <span>{t.resourcesPage.whitepapers.cta}</span>
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                          </svg>
                        </Link>
                        <span className="text-white/50 text-xs">
                          {language === 'zh' ? '需填写表单' : 'Form required'}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      <section className="py-8 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-2 font-lora">
              {t.resourcesPage.articles.title}
            </h2>
            <p className="text-white/80 max-w-3xl mx-auto text-sm md:text-base leading-relaxed">
