    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-slate-600 text-center">
          <h1 className="text-2xl font-bold mb-2 text-slate-800">{language === 'zh' ? '正在载入文章…' : 'Loading article…'}</h1>
  ...
  const renderMarkdown = (raw: string): string => {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (hMatch) {
        ...
        const cls = 'text-2xl md:text-3xl font-bold text-slate-900 mb-6 mt-12 first:mt-0';
      ...
      if (/^>\s?/.test(line)) {
        closeLists();
        if (!inBlockquote) { html.push('<blockquote class="border-l-4 border-emerald-200 pl-4 italic text-slate-700 bg-emerald-50/70 py-2 pr-4 rounded-r-lg my-6">'); inBlockquote = true; }
          ...
          <div className="bg-white rounded-3xl p-8 md:p-12 border border-slate-200 shadow-xl">
            {(() => {
              ...
              return (
                <>
                  {toc.length > 0 && (
                    <nav aria-label={language === 'zh' ? '目录' : 'Table of contents'} className="mb-10 rounded-2xl border border-slate-200 bg-slate-50 p-5">
                      <h2 className="text-slate-800 font-semibold mb-3 text-lg">{language === 'zh' ? '目录' : 'Contents'}</h2>
                      <ul className="space-y-2">
                        {toc.map((t, idx) => (
                          <li key={idx} className={t.level === 2 ? 'pl-0' : 'pl-4'}>
                            <a href={`#${t.id}`} className="text-emerald-600 hover:text-emerald-700 underline">
                              {t.text}
                            </a>
                          </li>
                        ))}
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-medium transition-all duration-300 hover:scale-105 flex items-center gap-2"
                  >
                  ...
                  {canWebShare && (
                    <button
                      ...
                      className="px-4 py-2 rounded-full bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-medium transition-all duration-300 hover:scale-105 flex items-center gap-2"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedArticles.map((relatedArticle) => (
                <Link
                  ...
                  className="bg-white rounded-2xl overflow-hidden border border-slate-200 hover:border-emerald-200 transition-all duration-300 hover:scale-105 group shadow-sm"
                >
