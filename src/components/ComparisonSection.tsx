...
type TabButtonProps = {
  id: 'ai' | 'consultant' | 'traditional';
  activeId: 'ai' | 'consultant' | 'traditional';
  setActive: (v: 'ai' | 'consultant' | 'traditional') => void;
  label: string;
};

const TabButton = ({ id, activeId, setActive, label }: TabButtonProps) => {
  const isActive = id === activeId;
  return (
    <button
      onClick={() => setActive(id)}
      className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
        isActive ? 'bg-white text-[rgb(0,52,50)]' : 'bg-white/10 text-white hover:bg-white/20'
      }`}
    >
      {label}
    </button>
...
const TimelinePanel = ({ title, meta, steps, collapsed, isZh, featured }: TimelinePanelProps) => {
